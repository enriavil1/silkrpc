...
class PerfTest:
    ...
    def start_silk_daemon(self, start_test):
        ...
        if self.config.user_perf_command != "" and start_test == 1:
            ...
            perf_cmd = ""
        if on_core[0] == "-":
            cmd = perf_cmd + self.config.silkrpc_build_dir + "silkrpc/silkrpcdaemon --target " + self.config.erigon_addr + " --eth1_local localhost:51515 --logLevel c &"
        else:
            cmd = perf_cmd + "taskset -c " + on_core[0] + " "\
                + self.config.silkrpc_build_dir + "silkrpc/silkrpcdaemon --target " + self.config.erigon_addr + " --eth1_local localhost:51515 --logLevel c  --numContexts "\
